...
class Bubble:
    ...
    def initialize_front(self):
        ...
        for i in range(1, self.point+2):
            self.x[i] = self.center_x-self.radius*math.sin(2.0*math.pi*(i-1)/self.point)
            self.y[i] = self.center_y-self.radius*math.cos(2.0*math.pi*(i-1)/self.point)